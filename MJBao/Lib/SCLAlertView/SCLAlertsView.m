...
@interface SCLAlertsView ()  <UITextFieldDelegate, UIGestureRecognizerDelegate>
...
- (void)setShouldDismissOnTapOutside:(BOOL)shouldDismissOnTapOutside
{
    ...
    if(_shouldDismissOnTapOutside)
    {
        ...
        [_backgroundView addGestureRecognizer:self.gestureRecognizer];
    }
}
...
- (SCLButton *)addButton:(NSString *)title
{
    ...
    btn.defaultBackgroundColor = self.buttonDefaultColor;
...
@property (strong, nonatomic) SCLAlertsView *alertView;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.alertView = [[SCLAlertsView alloc] init];
    }
    return self;
}
- (instancetype)initWithNewWindow {
    self = [super init];
    if (self) {
        self.alertView = [[SCLAlertsView alloc] initWithNewWindow];
    }
    return self;
}
...
- (instancetype)initWithNewWindowWidth:(CGFloat)width {
    self = [super init];
    if (self) {
        self.alertView = [[SCLAlertsView alloc] initWithNewWindowWidth:width];
    }
    return self;
}
...
#pragma mark - Show
@property(copy, nonatomic) void (^show)(SCLAlertsView *view, UIViewController *controller);
@end
...
- (void)showAlertView:(SCLAlertsView *)alertView {
    [self showAlertView:alertView onViewController:self.parameterViewController];
}

- (void)showAlertView:(SCLAlertsView *)alertView onViewController:(UIViewController *)controller {
...
- (void (^)(SCLAlertsView *view, UIViewController *controller))show {
    if (!_show) {
        __weak typeof(self) weakSelf = self;
        _show = ^(SCLAlertsView *view, UIViewController *controller) {
