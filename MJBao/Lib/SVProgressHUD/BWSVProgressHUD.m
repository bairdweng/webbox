...
@implementation BWSVProgressHUD {
    BOOL _isInitializing;
}

+ (BWSVProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static BWSVProgressHUD *sharedView;
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        NSBundle *bundle = [NSBundle bundleForClass:[BWSVProgressHUD class]];
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak BWSVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong BWSVProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
            // Stop timer
