...
@interface CSIAPManager () <SKProductsRequestDelegate, SKPaymentTransactionObserver>
...
@implementation CSIAPManager

+ (CSIAPManager *)sharedIAPManager {
    static CSIAPManager *sharedInstance;
    if(sharedInstance == nil) sharedInstance = [CSIAPManager new];
...
- (void)RefreshRequest:(RestorePurchasesCompletionBlock)completionBlock
{
...
#else
    if(! [self canPurchase])
        err([NSError errorWithDomain:@"CSIAPManager" code:0 userInfo:[NSDictionary dictionaryWithObject:@"Can't make payments" forKey:NSLocalizedDescriptionKey]]);
    else {
...
- (void)purchaseProduct:(SKProduct *)product ExtraStr:(NSString *)extrastr completion:(PurchaseCompletionBlock)completionBlock error:(ErrorBlock)err {
#ifdef SIMULATE_PURCHASES
    [self.purchasedItems addObject:product.productIdentifier];
    self.purchasedItemsChanged = YES;
    for(NSArray *t in self.purchasesChangedCallbacks) {
...
- (void)purchaseProductForId:(NSString *)productId completion:(PurchaseCompletionBlock)completionBlock error:(ErrorBlock)err {
    ...
    [self getProductsForIds:@[productId] completion:^(NSArray *products) {
        if([products count] == 0) err([NSError errorWithDomain:@"CSIAPManager" code:0 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Didn't find products with ID %@", productId] forKey:NSLocalizedDescriptionKey]]);
